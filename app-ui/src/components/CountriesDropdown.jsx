        ...
        <div className="dropdown">
            <button className="btn btn-secondary dropdown-toggle" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                {(window.location.pathname === "/")
                    ? "All countries"
                    : (window.location.pathname).split("/")[1].replaceAll("%20", " ").replaceAll("&nbsp;", " ")}
            </button>
            <div className="dropdown-menu scrollable-menu" aria-labelledby="dropdownMenuButton">
                {window.location.pathname !== "/" && <button className="dropdown-item" style={dropdownBtnStyle} onClick={handleSelect}>All countries</button>}
                {countries.map((c) => {
                    return (
